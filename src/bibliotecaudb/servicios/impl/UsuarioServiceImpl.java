...
public class UsuarioServiceImpl implements UsuarioService {
    ...
    @Override
public boolean eliminarUsuario(int idUsuarioAEliminar, Usuario actorQueElimina) throws SQLException, UsuarioException {
    if (actorQueElimina == null || actorQueElimina.getTipoUsuario() == null || !"Administrador".equals(actorQueElimina.getTipoUsuario().getTipo())) {
        throw new UsuarioException("No tiene permisos para eliminar usuarios.");
    }
    if (actorQueElimina.getId() == idUsuarioAEliminar) {
        throw new UsuarioException("Un administrador no puede eliminarse a si mismo a traves de esta funcion.");
    }

    Usuario usuarioAEliminar = usuarioDAO.obtenerPorId(idUsuarioAEliminar);
    if (usuarioAEliminar == null) {
        throw new UsuarioException("Usuario con ID " + idUsuarioAEliminar + " no encontrado para eliminar.");
    }
    
    // Se podrian añadir mas validaciones aqui, ej. no eliminar si tiene prestamos activos, etc.
    // Pero por ahora, el DAO intentara eliminarlo y la BD podria impedirlo si hay FKs.

    LogsError.info(this.getClass(), "Intentando eliminar usuario ID: " + idUsuarioAEliminar + " por actor: " + actorQueElimina.getCorreo());
    return usuarioDAO.eliminar(idUsuarioAEliminar);
}
}