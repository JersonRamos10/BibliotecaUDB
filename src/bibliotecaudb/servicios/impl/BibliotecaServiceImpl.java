...
public class BibliotecaServiceImpl implements BibliotecaService {
    ...
    @Override
    public Prestamo obtenerDetallesPrestamoParaDevolucion(int idPrestamo) throws SQLException, BibliotecaException {
        // Este metodo obtiene un prestamo por su ID para ver sus detalles, util antes de una devolucion.
        if (idPrestamo <= 0) { // Validacion basica del ID
            LogsError.warn(this.getClass(), "Intento de obtener detalles de prestamo con ID invalido: " + idPrestamo);
            throw new BibliotecaException("El ID del prestamo para obtener detalles no es valido.");
        }
        // El prestamoDAO.obtenerPorId ya deberia cargar los objetos Usuario y Ejemplar asociados
        Prestamo prestamo = prestamoDAO.obtenerPorId(idPrestamo);
        if (prestamo == null) {
            LogsError.warn(this.getClass(), "No se encontro prestamo con ID: " + idPrestamo + " al intentar obtener detalles para devolucion.");
            // Considera si aqui deberias lanzar una excepcion o si la GUI puede manejar un null.
            // Por consistencia con otros metodos "obtenerPorId", devolver null es aceptable y la GUI lo maneja.
        } else {
            LogsError.info(this.getClass(), "Obteniendo detalles para devolucion del prestamo ID: " + idPrestamo);
        }
        return prestamo; // Devuelve el prestamo encontrado, o null si no existe
    }

    @Override
    public boolean actualizarDatosDocumento(Documento documento) throws SQLException, BibliotecaException {
        // Este metodo sirve para actualizar la informacion principal de un documento que ya existe.
        if (documento == null || documento.getId() <= 0) {
            throw new BibliotecaException("Datos del documento invalidos para actualizar o ID no proporcionado.");
        }
        if (documento.getTitulo() == null || documento.getTitulo().trim().isEmpty()) {
            throw new BibliotecaException("El titulo del documento es obligatorio para la actualizacion.");
        }
        if (documento.getIdTipoDocumento() <= 0) { // El tipo de documento es importante
            throw new BibliotecaException("El tipo de documento es obligatorio y debe ser valido para la actualizacion.");
        }
        // Aqui podrias anadir mas validaciones si es necesario, por ejemplo:
        // - Verificar que el TipoDocumento exista en la BD.
        // - Validar el formato del anio de publicacion, etc.

        // Antes de actualizar, es buena practica verificar si el documento realmente existe.
        Documento docExistente = documentoDAO.obtenerPorId(documento.getId());
        if (docExistente == null) {
            throw new BibliotecaException("El documento con ID " + documento.getId() + " no existe y no puede ser actualizado.");
        }

        LogsError.info(this.getClass(), "Intentando actualizar datos del documento ID: " + documento.getId() + " con titulo: " + documento.getTitulo());
        boolean actualizado = documentoDAO.actualizar(documento); // Llamamos al metodo del DAO para actualizar
        if (actualizado) {
            LogsError.info(this.getClass(), "Documento ID: " + documento.getId() + " actualizado exitosamente.");
        } else {
            LogsError.warn(this.getClass(), "No se pudo actualizar el documento ID: " + documento.getId() + " (el DAO devolvio false).");
        }
        return actualizado;
    }
    ...
    @Override
    public PoliticasPrestamo obtenerPoliticaPorTipoUsuario(int idTipoUsuario) throws SQLException, BibliotecaException {
        // Este metodo obtiene la politica de prestamo para un tipo de usuario especifico.
        if (idTipoUsuario <= 0) {
            throw new BibliotecaException("ID de tipo de usuario no valido para obtener politica.");
        }
        return politicasPrestamoDAO.obtenerPorIdTipoUsuario(idTipoUsuario);
    }
}