...
public class MoraServiceImpl implements MoraService {
    ...
    public BigDecimal obtenerTasaMoraDiariaAplicable(LocalDate fechaParaCalculo) throws SQLException, BibliotecaException {
        ...
        int anio = fechaParaCalculo.getYear(); // Obtenemos el año de la fecha
        MoraAnual configMoraAnual = moraAnualDAO.obtenerPorAnio(anio); // Buscamos la mora para ese año
        ...
        if (configMoraAnual != null && configMoraAnual.getMoraDiaria() != null) {
            moraDiariaAplicable = configMoraAnual.getMoraDiaria(); // Usamos la mora de ese año
            LogsError.info(this.getClass(), "Tasa de mora anual (" + anio + ") encontrada: " + moraDiariaAplicable);
        } else {
            // Si no hay mora para ese año, usamos la mora global del sistema
    ...
    public MoraAnual obtenerMoraPorAnio(int anio) throws SQLException {
        // Este metodo busca la configuracion de mora para un año especifico.
    ...
    public boolean guardarMoraAnual(MoraAnual moraAnual) throws SQLException, BibliotecaException {
        // Este metodo guarda o actualiza la configuracion de mora para un año.
        if (moraAnual == null || moraAnual.getAnio() <= 1900 || moraAnual.getAnio() > LocalDate.now().getYear() + 10) { // Validacion simple del año
        ...
        MoraAnual existente = moraAnualDAO.obtenerPorAnio(moraAnual.getAnio()); // Vemos si ya existe una mora para ese año
        if (existente != null) {
    ...
    public boolean eliminarMoraAnual(int anio) throws SQLException {
        // Este metodo elimina la configuracion de mora para un año.
